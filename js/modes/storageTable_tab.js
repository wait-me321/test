...
layui.define(['laytpl', 'laypage', 'layer', 'form'], function(exports){
  ...
  //字符常量
  ,MOD_NAME = 'storageTable_tab', ELEM = '.layui-table', THIS = 'layui-this', SHOW = 'layui-show', HIDE = 'layui-hide', DISABLED = 'layui-disabled', NONE = 'layui-none'
  ...
  Class.prototype.pullData = function(curr, loadIndex){
    ...
    if(options.url){ //Ajax请求
      ...
      $.ajax({
        ...
        ,success: function(res){
          if(res[response.statusName] != response.statusCode){
            ...
            that.renderData(res, curr, res.data.totalElements), sort();
            options.time = (new Date().getTime() - that.startTime) + ' ms'; //耗时（接口请求+视图渲染）
          }
          loadIndex && layer.close(loadIndex);
          typeof options.done === 'function' && options.done(res, curr, res.data.totalElements);
        }
        ,error: function(e, m){
      ...
      res[response.clothingStockVOS] = options.data.concat().splice(startLimit, options.limit);
      res.data.totalElements = options.data.length;
  ...
  Class.prototype.renderData = function(res, curr, count, sort){
    var that = this
    ,options = that.config
    ,data = res.data.clothingStockVOS || []
    ,trs = []
    ,trs_fixed = []
    ,trs_fixed_r = []
