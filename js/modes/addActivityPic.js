layui.define(['layer', 'form', "table", "laypage", "element", "laytpl", "laydate","upload"], function (exports) {
    ...
    var activityPicDOTS=[]
    layer.msg('欢迎进入专题活动管理', { time: 1000, anim: 1 });
    var addActivityPicTable_option={
        ...
        , height: 'full'
        , method: "get"
        , url: 'http://api.xykoo.cn/manage/pic/searchActivityList' //数据接口
        ...
        , cols: [[ //表头
            { field: 'indexI', title: '序列', width: 100, edit:"text",  align: "center" ,sort:true }
            , { field: 'specialImg', title: '活动图片', width: 120, templet: "#specialImgTpl", align: "center" }
            , { field: 'specialUrl', title: '活动连接', width: 400,  edit:"text", align: "center" }
        ...
        ,done: function (obj,curr,count) {
            ...
            var specialImg = $(".layui-table-body [data-field=specialImg]")
            var caozuo = $(".layui-table-fixed-r .layui-table-body  [data-field=caozuo]")

            $(window).resize(function () {
                specialImg.each(function (index, obj) {
                    caozuo.eq(index).css("height", specialImg.eq(index).css("height"))
                });
            ...
            function intImg(){
                ...
                if($(".layui-table-body .selectImg").get(0)){
                ...
                $("[data-field=specialImg]").on('click', function(){
                    img=$(this).find("img")
                    $(".file_img").click()
                });
            }
                ...
                switch(obj.event){
                  case 'save':
                    ...
                    layer.confirm("确定保存更改吗？",function(){
                        ...
                        for(var i in activityPicDOTS){
                            var json={
                                activityPicId:activityPicDOTS[i].specialId,
                                imgFile:activityPicDOTS[i].specialImg,
                                linkUrl:activityPicDOTS[i].specialUrl
                            }
                            subdata.push(json)
                            delete(activityPicDOTS[i]["indexI"])
                            delete(activityPicDOTS[i]["LAY_TABLE_INDEX"])
                        }

                        active.saveActivity(subdata)
                    })
                  break;
                  case 'addActivity':
                    active.addActivity()
            ...
            table.on('tool(addActivityPicTable)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
              ...
              if(layEvent === 'del'){ //删除
                layer.confirm('真的删除吗？', function(index){
                    layer.close(index)
                    active.deleteActivityPic(data.specialId)
                    $(obj.tr).remove()
    ...
    active={
        ...
        ,deleteActivityPic: function (id) {
            ...
            var index=layer.load(2)
            $.ajax({
                url: "http://api.xykoo.cn/manage/pic/deleteActivityPic",
        ...
        saveActivity:function(activityData,opindex){
            var activityData_Data=JSON.parse(JSON.stringify(activityData))
            $.each(activityData,function(key,val){
                val.activityPicId=0
            })
            var index=layer.load(2)
            $.ajax({
                url: "http://api.xykoo.cn/manage/pic/addActivityPic",
                beforeSend: function (request) {
                    request.setRequestHeader("X-Auth-Token", token);
                },
                type: "post",
                ...
                success: function (data) {
                    ...
                    switch (data.status) {
                        case 200:
                            layer.close(opindex)
                            $.each(activityData_Data,function(key,val){
                                
                                    active.deleteActivityPic(val.activityPicId)
