layui.define(['layer', 'form', "table", "laypage", "element", "laytpl", "laydate","upload"], function (exports) {
    ...
    var activityPicDOTS=[]
    layer.msg('欢迎进入热门穿搭管理', { time: 1000, anim: 1 });
    var addActivityPicTable_option={
        ...
        , height: 'full'
        , method: "get"
        , url: 'http://api.xykoo.cn/manage/pic/searchHotWearList' //数据接口
        ...
        , cols: [[ //表头
            ...
            , { field: 'hotWearImg', title: '轮播图片', width: 120, templet: "#specialImgTpl", align: "center" }
            , { field: 'hotWearUrl', title: '轮播连接', width: 400,  edit:"text", align: "center" }
        ...
        ,done: function (obj,curr,count) {
            ...
            var hotWearImg = $(".layui-table-body [data-field=hotWearImg]")
            var caozuo = $(".layui-table-fixed-r .layui-table-body  [data-field=caozuo]")

            $(window).resize(function () {
                hotWearImg.each(function (index, obj) {
                    caozuo.eq(index).css("height", hotWearImg.eq(index).css("height"))
                });
            ...
            function intImg(){
                ...
                $("[data-field=hotWearImg]").on('click', function(){
                    img=$(this).find("img")
                    $(".file_img").click()
                });
            }
            ...
            table.on('row(addActivityPicTable)', function(obj){
                ...
                obj.data.hotWearImg=obj.tr.find("[data-field=hotWearImg] img").attr("src")
                ...
                switch(obj.event){
                  case 'save':
                    ...
                    layer.confirm("确定保存更改吗？",function(){
                        ...
                        for(var i in activityPicDOTS){
                            var json={
                                hotWearId:activityPicDOTS[i].hotWearId,
                                imgFile:activityPicDOTS[i].hotWearImg,
                                linkUrl:activityPicDOTS[i].hotWearUrl
                            }
                            subdata.push(json)
                            delete(activityPicDOTS[i]["indexI"])
            ...
            table.on('tool(addActivityPicTable)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
              ...
              if(layEvent === 'del'){ //删除
                layer.confirm('真的删除吗？', function(index){
                    layer.close(index)
                    active.deleteActivityPic(data.hotWearId)
                    $(obj.tr).remove()
    ...
    active={
        ...
        ,deleteActivityPic: function (id) {
            ...
            var index=layer.load(2)
            $.ajax({
                url: "http://api.xykoo.cn/manage/pic/deleteHotWearPic",
        ...
        saveActivity:function(activityData,opindex){
            ...
            var index=layer.load(2)
            $.ajax({
                url: "http://api.xykoo.cn/manage/pic/addHotWearPic",
                beforeSend: function (request) {
                    request.setRequestHeader("X-Auth-Token", token);
                },
                type: "post",
                    ...
                    switch (data.status) {
                        case 200:
                            ...
                            $.each(activityData_Data,function(key,val){
                                    ...
                                    active.deleteActivityPic(val.hotWearId)
