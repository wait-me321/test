layui.define(['layer', 'form', "table", "laypage", "element", "laytpl", "laydate","upload"], function (exports) {
    ...
    var activityPicDOTS=[]
    layer.msg('欢迎进入首页轮播图管理', { time: 1000, anim: 1 });
    var addActivityPicTable_option={
        ...
        , height: 'full'
        , method: "get"
        , url: 'http://api.xykoo.cn/manage/pic/searchHomeImgList' //数据接口
        ...
        , cols: [[ //表头
            ...
            , { field: 'homeImgUrl', title: '轮播图片', width: 120, templet: "#specialImgTpl", align: "center" }
            , { field: 'homeLinkUrl', title: '轮播连接', width: 400,  edit:"text", align: "center" }
        ...
        ,done: function (obj,curr,count) {
            ...
            var homeImgUrl = $(".layui-table-body [data-field=homeImgUrl]")
            var caozuo = $(".layui-table-fixed-r .layui-table-body  [data-field=caozuo]")

            $(window).resize(function () {
                homeImgUrl.each(function (index, obj) {
                    caozuo.eq(index).css("height", homeImgUrl.eq(index).css("height"))
                });
            ...
            function intImg(){
                ...
                $("[data-field=homeImgUrl]").on('click', function(){
                    img=$(this).find("img")
                    $(".file_img").click()
                });
            }
            ...
            table.on('row(addActivityPicTable)', function(obj){
                ...
                obj.data.homeImgUrl=obj.tr.find("[data-field=homeImgUrl] img").attr("src")
                ...
                switch(obj.event){
                  case 'save':
                    ...
                    layer.confirm("确定保存更改吗？",function(){
                        ...
                        for(var i in activityPicDOTS){
                            var json={
                                loopPicId:activityPicDOTS[i].homeImgId,
                                imgFile:activityPicDOTS[i].homeImgUrl,
                                linkUrl:activityPicDOTS[i].homeLinkUrl
                            }
                            subdata.push(json)
                            delete(activityPicDOTS[i]["indexI"])
            ...
            table.on('tool(addActivityPicTable)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
              ...
              if(layEvent === 'del'){ //删除
                layer.confirm('真的删除吗？', function(index){
                    layer.close(index)
                    active.deleteActivityPic(data.homeImgId)
                    $(obj.tr).remove()
    ...
    active={
        ...
        ,deleteActivityPic: function (id) {
            ...
            var index=layer.load(2)
            $.ajax({
                url: "http://api.xykoo.cn/manage/pic/deleteLoopPic",
        ...
        saveActivity:function(activityData,opindex){
            ...
            var index=layer.load(2)
            $.ajax({
                url: "http://api.xykoo.cn/manage/pic/addLoopPic",
                beforeSend: function (request) {
                    request.setRequestHeader("X-Auth-Token", token);
                },
                type: "post",
                    ...
                    switch (data.status) {
                        case 200:
                            ...
                            $.each(activityData_Data,function(key,val){
                                    ...
                                    active.deleteActivityPic(val.loopPicId)
